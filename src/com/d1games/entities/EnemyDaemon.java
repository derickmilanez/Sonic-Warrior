...
public class EnemyDaemon extends Entity {
	
	private double speed = 6;
	private int frames = 0, maxFrames = 20, index = 0, maxIndex = 1;
	private int maskx = 2, masky = 0, maskw = 28, maskh = 27;
	private BufferedImage[] sprites;
	private int life = 5;
	private boolean isDamaged = false;
	private int damageFrames = 10, damageCurrent = 0;

	public EnemyDaemon(int x, int y, int width, int height, BufferedImage sprite) {
		super(x, y, width, height, null);
		sprites = new BufferedImage[2];
		sprites[0] = Game.spritesheet.getSprite(0, 160, 32, 32);
		sprites[1] = Game.spritesheet.getSprite(32, 160, 32, 32);
	}
	...
	public void render(Graphics g) {
		...
		//g.fillRect(this.getX() + maskx - Camera.x, this.getY() + masky - Camera.y, maskw, maskh);
			...
			g.drawImage(Entity.DAEMON_FEEDBACK, this.getX() - Camera.x, this.getY() - Camera.y, null);
		}
	}
}
